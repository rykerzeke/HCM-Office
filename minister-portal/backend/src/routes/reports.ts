...
export default async function reportRoutes(fastify: FastifyInstance) {
  fastify.get('/reports', {
    ...
    try {
      const [districtWise, workload] = await Promise.all([
        prisma.case.groupBy({ by: ['status'], _count: { id: true } }),
        prisma.assignment.groupBy({ by: ['userId'], _count: { id: true } }),
      ]);
      return reply.send({ districtWise, workload });
    } catch (err) {
  ...
  fastify.get('/reports/pdf', {
    ...
    try {
      const [districtWise, workload, cases] = await Promise.all([
        prisma.case.groupBy({ by: ['status'], _count: { id: true } }),
        prisma.assignment.groupBy({ by: ['userId'], _count: { id: true } }),
  ...
  fastify.get('/reports/excel', {
    ...
    try {
      ...
      wsStatus.addRows(districtWise.map((r) => ({ status: r.status.replace(/_/g, ' '), count: r._count.id })));
      ...
      wsWorkload.addRows(workload.map((r) => ({ userId: r.userId, count: r._count.id })));
